...
| App drawer icons overlap the A–Z letter index | Icons on the right edge of the drawer overlap the letter scrubber strip (`AlphabetIndexView`). Needs additional right padding on the drawer RecyclerView so icons clear the strip. |
| No visual separator between letter groups in app drawer | The alphabetical section headers (`AnchorAlphabeticalAppsList` / `VIEW_TYPE_SECTION_HEADER`) have no divider/rule between groups, so it's not obvious where one letter ends and the next begins. Add a separator/divider line between letter groups. |
