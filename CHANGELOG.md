...
### Changed
- **Home Screen settings reorganized** into clearer groups. Cell spacing now sits with the grid options under Layout; the Icons group (size, labels) moved directly beneath it; the setup-wizard entry moved to the end of Layout. The catch-all "Anchor Launcher" section is split into logical headings: the rotation-stable wallpaper settings are merged into the standard **Wallpaper** group behind a single "Use Anchor to set wallpaper" master toggle (which reveals the image picker, parallax and test options only when on), and rotation-transition and drawer-access settings get their own **Rotation** and **App drawer access** groups. The rotation-animation options are now self-describing ("Traditional (system default) / Instant (no animation) / Fade (hide & reveal)") with an explanatory line, and the parallax slider gained a description.
...
### Fixed
- **Icons and labels were too small on large tablets, and the size slider couldn't compensate.** Icon size was purely density-relative (a fixed dp value), so on a physically large but low-density tablet a "normal" icon was a much smaller fraction of the screen than on a phone (~6 % vs ~12 % of the short side), and the recommendation engine then packed far too many tiny columns. Even the maximum icon-size setting couldn't reach phone-like proportions. Icon sizing is now **screen-relative**: it is floored at a fraction of the screen's short side, so large screens get proportionally larger icons and sensible column counts. Phones are unaffected (their dp size already exceeds the floor). Applies to both the setup-wizard recommendation and the live rendered grid.
- **Setup-wizard grid preview now shows the correct layout in landscape.** Previously it always drew a portrait card; on a device held in landscape it now shows the transposed grid (rows × columns) in a landscape-shaped card, with icons kept upright and each landing at its true rotated position (portrait top-left → landscape bottom-left), so the preview matches what the home screen actually does on rotation. Fixed a related overflow where the preview grid spilled past the card edge.
- **Icons landed at the wrong position after rotating inside another app.** If the screen rotated while the launcher was backgrounded (e.g. rotating inside YouTube or a camera app) and settled before you returned home, the deferred grid transpose never ran, so icons stayed in the previous orientation's positions. The launcher now reconciles the rotation on resume — if the recorded rotation differs from the current one, it performs the transpose immediately. A related latent bug where the transpose could use stale grid dimensions (the portrait dims are now cached authoritatively) is also fixed.
